 ...
 * dark run: the reviews band above it is the dark one, and the closing CTA
 * below it is dark again.
 *
 * Three stacked cards ran to about nine hundred pixels on a phone, for a
 * band that is not on the way to an enquiry. It swipes there instead, in the
 * same rail the reviews use, and only opens out to a row of three from the
 * large breakpoint.
 */
  ...
  return (
    <section className="bg-paper text-ink">
      <div className="mx-auto max-w-6xl px-4 py-[50px] sm:px-6">
        ...
        <ul ref={rail} className={`${RAIL_CLASS} mt-12`}>
          {latest.map((post) => (
            <li
              key={post.slug}
              className="flex w-[82%] flex-none snap-start sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)]"
            >
              <article className="group relative flex w-full flex-col overflow-hidden rounded-2xl border border-[rgba(212,175,55,0.34)] bg-white shadow-[0_10px_30px_rgba(0,0,0,0.04)] transition-all duration-300 hover:-translate-y-1.5 hover:border-gold hover:shadow-[0_18px_40px_rgba(212,175,55,0.2)]">
                <div className="relative aspect-[3/2] overflow-hidden bg-sand">
                  ...
                  />
                  <span className="absolute right-3.5 top-3.5 z-10 rounded-full bg-[linear-gradient(180deg,#CBA55C,#A37F34)] px-3.5 py-1.5 text-[12px] font-bold text-white shadow-[0_8px_20px_-10px_rgba(163,127,52,0.9)]">
                    {post.tag}
                  </span>
                </div>
